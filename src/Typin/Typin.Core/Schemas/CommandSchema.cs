    ...
    public class CommandSchema
    {
        ...
        private void BindParameters(ICommand instance, IReadOnlyList<CommandParameterInput> parameterInputs)
        {
            ...
            for (int i = 0; i < scalarParameters.Length; i++)
            {
                ...
                CommandParameterInput scalarInput = i < parameterInputs.Count
                                                        ? parameterInputs[i]
                                                        : throw EndUserExceptions.ParameterNotSet(parameter);
            ...
            if (nonScalarParameter != null)
            {
                ...
                if (!nonScalarValues.Any())
                    throw EndUserExceptions.ParameterNotSet(nonScalarParameter);
            ...
            if (remainingParameterInputs.Any())
                throw EndUserExceptions.UnrecognizedParametersProvided(remainingParameterInputs);
        }
                                 ...
                                 IOptionFallbackProvider optionFallbackProvider)
        {
            ...
            if (remainingOptionInputs.Any())
                throw EndUserExceptions.UnrecognizedOptionsProvided(remainingOptionInputs);

            // Ensure all required options were set
            if (unsetRequiredOptions.Any())
                throw EndUserExceptions.RequiredOptionsNotSet(unsetRequiredOptions);
        }

        /// <summary>
        /// Binds input in command instance.
        /// </summary>
        public void Bind(ICommand instance,
                         CommandInput input,
                         IOptionFallbackProvider optionFallbackProvider)
        {
